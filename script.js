document.addEventListener("DOMContentLoaded", () => {
  ...
  function renderMedicines(){
    if(!medicineList||!medicineHistory)return;
    const today=medicineTodayRecord();
    const taken=new Set(Array.isArray(today?.taken_ids)?today.taken_ids:[]);
    medicineList.innerHTML="";
    if(!medicines.length){medicineList.innerHTML='<div class="medicine-empty">Список ліків порожній. Додайте перші ліки або добавку.</div>';}
    medicines.forEach(m=>{
      ...
      const name=document.createElement("span");name.textContent=m.name;label.append(check,name);
  ...
  medicineSaveDay?.addEventListener("click",()=>{
    if(!medicines.length){showButtonState(medicineSaveDay,"Немає ліків","error",1500);logAction("День ліків не записано: список ліків порожній.");return;}
    const checked=[...medicineList.querySelectorAll('input[type="checkbox"]:checked')].map(x=>x.dataset.id);
    ...
    saveMedicineArchiveLocal();renderMedicines();showButtonState(medicineSaveDay,"День записано","success",1600);
