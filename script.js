document.addEventListener("DOMContentLoaded", () => {
  ...
  const medicineNameInput = $("medicine-name");
  const medicineAddButton = $("medicine-add");
  ...
  function exportFingerprint(){
    return JSON.stringify({products:products.map((p,i)=>normalizeProduct(p,i)),archive:archiveItems,medicines,medicine_archive:medicineArchive});
  }
  ...
  exportButton?.addEventListener("click",()=>{
    ...
    const data={version,exported_at:new Date().toISOString(),products:products.map((p,i)=>normalizeProduct(p,i)),archive:archiveItems,medicines,medicine_archive:medicineArchive};
  ...
  importFile?.addEventListener("change",async()=>{
    ...
    try{
      ...
      saveProductsLocal();if(hasArchive)saveArchiveLocal();if(hasMedicines)saveMedicinesLocal();if(hasMedicineArchive)saveMedicineArchiveLocal();
  ...
  function closeMedicineSelectModal(){
    medicineSelectModal?.classList.remove("active");
    medicineSelectModal?.setAttribute("aria-hidden","true");
  ...
  function openMedicineSelectModal(){
    ...
    const today=medicineTodayRecord();
    const taken=new Set(Array.isArray(today?.taken_ids)?today.taken_ids:[]);
    medicineSelectList.innerHTML="";
    medicines.forEach(m=>{
      const row=document.createElement("div");row.className="medicine-row medicine-select-row";
      const label=document.createElement("label");label.className="medicine-check";
      const check=document.createElement("input");check.type="checkbox";check.checked=taken.has(m.id);check.dataset.id=m.id;
      const name=document.createElement("span");name.textContent=m.name;label.append(check,name);row.append(label);medicineSelectList.append(row);
    });
    medicineSelectModal.classList.add("active");medicineSelectModal.setAttribute("aria-hidden","false");document.body.classList.add("edit-modal-open");
  }
  function renderMedicines(){
    if(!medicineBaseList||!medicineHistory)return;
    const today=medicineTodayRecord();
    medicineBaseList.innerHTML="";
    if(!medicines.length){medicineBaseList.innerHTML='<div class="medicine-empty">База ліків порожня. Додайте перші ліки або добавку.</div>';}
    medicines.forEach(m=>{
      const row=document.createElement("div");row.className="medicine-row medicine-base-item";
      const name=document.createElement("span");name.className="medicine-base-name";name.textContent=m.name;
      const remove=document.createElement("button");remove.type="button";remove.className="medicine-remove";remove.textContent="Видалити";
      remove.onclick=()=>{if(!confirm(`Видалити «${m.name}» з бази ліків?`)){showButtonState(remove,"Не видалено","error",1300);logAction(`Видалення ліків «${m.name}» скасовано.`);return;}medicines=medicines.filter(x=>x.id!==m.id);saveMedicinesLocal();renderMedicines();showButtonState(medicineAddButton,"Видалено","success",1200);logAction(`Видалено ліки/добавку «${m.name}».`);};
      row.append(name,remove);medicineBaseList.append(row);
    });
    if(medicineTodaySummary){
      if(!medicines.length) medicineTodaySummary.textContent="Спочатку додайте ліки до своєї бази.";
      else if(!today) medicineTodaySummary.textContent="За сьогодні ліки ще не записані.";
      else {
        const takenNames=Array.isArray(today.taken_names)?today.taken_names:[];
        if(!takenNames.length) medicineTodaySummary.textContent="За сьогодні жодні ліки не відмічені як прийняті.";
        else medicineTodaySummary.textContent=`Сьогодні записано: ${takenNames.join(", ")}.`;
      }
    }
    medicineHistory.innerHTML="";
    if(!medicineArchive.length){medicineHistory.innerHTML='<div class="medicine-empty">Архів ліків порожній.</div>';return;}
    [...medicineArchive].sort((a,b)=>String(b.date).localeCompare(String(a.date))).forEach(r=>{
      const names=new Map(medicines.map(m=>[m.id,m.name]));
      const allNames=Array.isArray(r.all_names)?r.all_names:[];
      const takenNames=Array.isArray(r.taken_names)?r.taken_names:(r.taken_ids||[]).map(id=>names.get(id)).filter(Boolean);
      const missedNames=allNames.filter(n=>!takenNames.includes(n));
      let status="Ліки зовсім не записані.";
      if(allNames.length&&takenNames.length===allNames.length)status="Всі ліки записано.";
      else if(takenNames.length)status=`Записані: ${takenNames.join(", ")}. Не записані: ${missedNames.join(", ") || "—"}.`;
      const row=document.createElement("div");row.className="medicine-history-item";
      const date=document.createElement("strong");date.textContent=formatArchiveDate(r.date);
      const text=document.createElement("div");text.textContent=status;row.append(date,text);medicineHistory.append(row);
    });
  }
  medicineAddButton?.addEventListener("click",()=>{
    const name=medicineNameInput.value.trim();
    if(!name){showButtonState(medicineAddButton,"Введіть назву","error",1400);logAction("Ліки не додано: назву не вказано.");medicineNameInput.focus();return;}
    if(medicines.some(m=>m.name.toLowerCase()===name.toLowerCase())){showButtonState(medicineAddButton,"Вже є","error",1400);logAction(`Ліки «${name}» не додано повторно.`);return;}
    medicines.push({id:createId("medicine"),name,created_at:new Date().toISOString()});saveMedicinesLocal();medicineNameInput.value="";renderMedicines();showButtonState(medicineAddButton,"Додано","success",1400);logAction(`Додано ліки/добавку «${name}».`);
  });
  medicineNameInput?.addEventListener("keydown",e=>{if(e.key==="Enter"){e.preventDefault();medicineAddButton.click();}});
  medicineOpenToday?.addEventListener("click",openMedicineSelectModal);
  medicineSelectCancel?.addEventListener("click",()=>{closeMedicineSelectModal();showButtonState(medicineSelectCancel,"Скасовано","error",1200);logAction("Додавання ліків за сьогодні скасовано.");});
  medicineSelectModal?.addEventListener("click",e=>{if(e.target===medicineSelectModal)closeMedicineSelectModal();});
  medicineSaveDay?.addEventListener("click",()=>{
    if(!medicines.length){showButtonState(medicineSaveDay,"Немає ліків","error",1500);logAction("День ліків не записано: база ліків порожня.");return;}
    const checked=[...medicineSelectList.querySelectorAll('input[type="checkbox"]:checked')].map(x=>x.dataset.id);
    const allNames=medicines.map(m=>m.name);const takenNames=medicines.filter(m=>checked.includes(m.id)).map(m=>m.name);
    const rec={date:getCurrentDate(),taken_ids:checked,all_names:allNames,taken_names:takenNames,updated_at:new Date().toISOString()};
    const i=medicineArchive.findIndex(r=>r.date===rec.date);if(i>=0)medicineArchive[i]=rec;else medicineArchive.unshift(rec);
    saveMedicineArchiveLocal();closeMedicineSelectModal();renderMedicines();showButtonState(medicineOpenToday,"Додано","success",1600);
    if(!takenNames.length)logAction("Ліки за сьогодні записані: жодні ліки не прийнято.");else if(takenNames.length===allNames.length)logAction("Ліки за сьогодні записані: всі ліки прийнято.");else logAction(`Ліки за сьогодні записані частково: ${takenNames.join(", ")}.`);
  });
  ...
  leaveSiteButton?.addEventListener("click",()=>{
    ...
    const keys=[PRODUCTS_KEY,CALCULATOR_KEY,ARCHIVE_KEY,ACTIVE_TAB_KEY,CALCULATOR_DRAFT_KEY,SORT_KEY,CONSOLE_KEY,EXPORT_VERSION_KEY,DATABASE_UPDATED_KEY,EXPORT_FINGERPRINT_KEY,MEDICINES_KEY,MEDICINE_ARCHIVE_KEY];
